...
{
    public partial class MySQLApp : Form
    {
        ...
        public MySQLApp()
        {
            ...
            connStringsDict = UpdateConnectionsComboBox(ComboBoxSQLConnecties, connectionStringSettings);
            mySqlConnStr = ComboBoxSQLConnecties.SelectedValue.ToString();
        }
        ...
        private void ComboBoxSQLConnecties_SelectedIndexChanged(object sender, EventArgs e)
        {
            mySqlConnStr = ComboBoxSQLConnecties.SelectedValue.ToString();
            ...
            try
            {
                using (mySqlConn = OpenMySQLverbinding(mySqlConnStr))
                {
                    if (mySqlConn.State == ConnectionState.Open)
                    {
                        ...
                        mySqlComm.CommandText = "select * from bestelling;";
                        ...
                        using (MySqlDataReader mySqlDr = mySqlComm.ExecuteReader())
                        {
                            while (mySqlDr.Read())
                            {
                                textBox1.Text += (int)mySqlDr[0] + "\t" + Convert.ToString((DateTime)mySqlDr[1]) + "\t" + (int)mySqlDr[2] + "\t" + (int)mySqlDr[3] + "\t" + (int)mySqlDr[4] + "\t" + mySqlDr[5] + "\r\n";
                            }
